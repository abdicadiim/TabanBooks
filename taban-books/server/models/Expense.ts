/**
 * Expense Model
 * Expenses - Based on OpenAPI spec
 */
...
export interface IExpense extends Document {
  ...
  expense_id?: string; // For compatibility with OpenAPI
  ...
  recurring_expense_id?: mongoose.Types.ObjectId;
...
const expenseSchema = new Schema<IExpense>(
  {
    ...
    expense_id: String,
    transaction_id: String,
    transaction_type: { type: String, default: "expense" },
    ...
    status: {
      type: String,
      enum: ["unbilled", "invoiced", "reimbursed", "non-billable", "billable"],
      default: "unbilled",
    },
    ...
    recurring_expense_id: mongoose.Schema.Types.ObjectId,
    journalEntryId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "JournalEntry"
    },
    ...
    line_items: [expenseLineItemSchema],
    taxes: [{
    ...
    // Populated fields (stored for quick access)
    account_name: String,
    paid_through_account_name: String,
    customer_name: String,
    vendor_name: String,
...
expenseSchema.pre('save', function (next) {
  if (!this.expense_id) {
    // Generate a unique expense_id to avoid duplicates
    const timestamp = Date.now();
    const random = Math.floor(Math.random() * 1000);
    this.expense_id = `EXP-${timestamp}-${random}`;
  }
...
expenseSchema.index({ organization: 1, account_name: 1 });
expenseSchema.index({ organization: 1, createdAt: -1 });
expenseSchema.index({ organization: 1, updatedAt: -1 });
expenseSchema.index({ organization: 1, date: 1, amount: -1 });
expenseSchema.index({ organization: 1, account_id: 1 });
expenseSchema.index({ organization: 1, vendor_id: 1 });
expenseSchema.index({ organization: 1, customer_id: 1 });
expenseSchema.index({ organization: 1, status: 1 });
expenseSchema.index({ organization: 1, reference_number: 1 });
expenseSchema.index({ organization: 1, project_id: 1 });
expenseSchema.index({ organization: 1, is_billable: 1 });
expenseSchema.index({ organization: 1, created_time: 1 });
registerDashboardHomeInvalidationHooks(expenseSchema);
