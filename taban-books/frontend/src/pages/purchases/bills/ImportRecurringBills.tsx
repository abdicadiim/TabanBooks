...
import { recurringBillsAPI, vendorsAPI } from "../../../services/api";
...
const REQUIRED_FIELDS = ["Profile Name", "Repeat Every", "Start Date", "Vendor Name", "Amount"];
const CURRENCY_CODE_REGEX = /^[A-Z]{3}$/;

const FIELD_ALIASES: Record<string, string[]> = {
  "Profile Name": ["ProfileName", "Profile", "Name"],
  "Repeat Every": ["Frequency", "Repeat", "RepeatEvery"],
  "Start Date": ["StartDate", "Date", "Bill Date"],
  "Ends On": ["End Date", "EndDate", "EndsOn"],
  "Never Expires": ["NeverExpire", "Never Expires?"],
  "Bill#": ["Bill Number", "Recurring Bill Number", "RecurringBillNo"],
  "Vendor Name": ["Vendor", "VendorName", "Supplier", "Supplier Name"],
  Amount: ["Total", "Bill Amount", "Recurring Amount"],
  "Payment Terms": ["Terms", "PaymentTerms"],
  "Currency Code": ["Currency", "CurrencyCode"],
  Status: ["Recurring Status"],
  Notes: ["Description", "Memo"],
  "Item Name": ["Item", "Item Details"],
  "Item Description": ["Line Description", "Description"],
  Quantity: ["Qty"],
  Rate: ["Unit Price", "UnitPrice", "Price"],
  Tax: ["Tax Amount", "Tax Total"],
  Discount: ["Discount Amount"],
  "Accounts Payable": ["Accounts Payable Account", "AP Account"],
};

const normalizeText = (value: any) => String(value || "").trim().toLowerCase();
const normalizeFieldText = (value: string) => String(value || "").toLowerCase().replace(/[^a-z0-9]/g, "");
const isMongoObjectId = (value: any) => /^[a-fA-F0-9]{24}$/.test(String(value || "").trim());
...
const normalizeRepeatEvery = (value: any) => {
  ...
  if (compact.endsWith("day") || compact.endsWith("days")) {
    const count = Number.parseInt(compact.replace(/[^0-9]/g, "") || "1", 10);
    return `${Math.max(1, count)} days`;
  }
  return String(value || "Month").trim() || "Month";
};

const parseNeverExpire = (value: any): boolean | null => {
  const normalized = normalizeText(value);
...
const normalizeStatus = (value: any): string => {
  ...
  if (["stopped", "stop", "inactive", "paused"].includes(normalized)) return "stopped";
  if (["expired"].includes(normalized)) return "expired";
  return "active";
};

const findVendorByName = (items: any[], value: string) => {
...
export default function ImportRecurringBills() {
  ...
  const previewSkippedRecords = useMemo(
    () =>
      parsedRows.reduce((acc: { rowNumber: number; reason: string }[], row, index) => {
        ...
        if (!profileName || !repeatEvery || !startDate || !vendorName || !amountRaw) {
    ...
    try {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        try {
          ...
          const vendorNameRaw = getRowValue(row, "Vendor Name");
          const amountRaw = getRowValue(row, "Amount");

          if (!profileName || !repeatEveryRaw || !startDateRaw || !vendorNameRaw || !amountRaw) {
            skippedCount += 1;
            continue;
          }
          ...
          const vendor = findVendorByName(vendors, vendorNameRaw);
          const vendorIdCandidate = vendor?._id || vendor?.id;
          const vendorId = isMongoObjectId(vendorIdCandidate) ? String(vendorIdCandidate) : undefined;

          const repeatEvery = normalizeRepeatEvery(repeatEveryRaw);
          const parsedNeverExpire = parseNeverExpire(getRowValue(row, "Never Expires"));
          const neverExpire = parsedNeverExpire == null ? !endDate : parsedNeverExpire;
          const quantityRaw = parseAmount(getRowValue(row, "Quantity"));
          const rateRaw = parseAmount(getRowValue(row, "Rate"));
          ...
          const rate =
            Number.isFinite(rateRaw) && rateRaw >= 0
              ...
              : quantity > 0
                ? amount / quantity
                : amount;
          const taxAmount = parseAmount(getRowValue(row, "Tax"));
          const discountAmount = parseAmount(getRowValue(row, "Discount"));
          const currencyCode = String(getRowValue(row, "Currency Code") || "").trim().toUpperCase();

          const payload: any = {
            profile_name: profileName,
            recurring_bill_id: getRowValue(row, "Bill#") || undefined,
            repeat_every: repeatEvery,
            start_date: startDate,
            vendor: vendorId,
            vendor_name: vendor?.displayName || vendor?.name || vendor?.companyName || vendor?.company || vendorNameRaw,
            paymentTerms: getRowValue(row, "Payment Terms") || undefined,
            ...
            total: amount,
            amount,
            accounts_payable: getRowValue(row, "Accounts Payable") || undefined,
            items: [
              {
                name: getRowValue(row, "Item Name") || "Imported recurring bill",
                description: getRowValue(row, "Item Description") || getRowValue(row, "Notes") || "",
                quantity,
                unitPrice: rate,
                total: amount,
              },
          ...
          if (CURRENCY_CODE_REGEX.test(currencyCode)) payload.currency = currencyCode;

          const response = await recurringBillsAPI.create(payload);
          const success = response?.code === 0 || response?.success || response?.recurring_bill || response?.data;
      ...
      console.error("Recurring bill import failed:", error);
    ...
    <div className="flex flex-col h-screen bg-white">
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50">
        <div className="flex items-center gap-4">
          <h1 className="text-lg font-semibold text-gray-900 m-0">{currentStep === 1 ? "Recurring Bills - Select File" : currentStep === 2 ? "Map Fields" : "Preview"}</h1>
        ...
        <button onClick={() => navigate("/purchases/recurring-bills")} className="p-2 bg-transparent border-none cursor-pointer"><X size={20} className="text-red-600" /></button>
      </div>
      ...
      <div className={`p-8 overflow-y-auto flex-1 ${currentStep === 1 ? "max-w-2xl" : "max-w-5xl"} mx-auto w-full`}>
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">Recurring Bill Details</div>
        ...
        {currentStep === 3 && (
          <div className="space-y-5">
            ...
            <div className="bg-blue-50 border border-blue-100 text-blue-800 text-sm px-4 py-3 rounded-md flex items-start gap-2"><Info size={16} className="mt-0.5 shrink-0" /><span>Click Previous to adjust mappings, or click Import to write these recurring bills to the database.</span></div>
      ...
      <div className="px-8 py-4 border-t border-gray-200 flex items-center justify-between bg-white">
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate("/purchases/recurring-bills")} className="px-4 py-2 text-sm border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-50">Cancel</button>
