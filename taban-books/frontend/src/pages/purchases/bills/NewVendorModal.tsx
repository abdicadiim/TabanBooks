...
export default function NewVendorModal({ isOpen, onClose, onVendorCreated }: { isOpen: boolean; onClose: () => void; onVendorCreated: (vendor: any) => void }) {
  ...
  function generateDisplayNameOptions(data: Partial<{ firstName: string; lastName: string; companyName: string }>) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newVendor = {
      ...
      name: formData.displayName || formData.firstName + " " + formData.lastName || formData.companyName,
      companyName: formData.companyName || "",
      email: formData.email || "",
      workPhone: formData.workPhone || "",
      payables: "AWG0.00",
      unusedCredits: "AWG0.00",
      formData: {
        ...formData,
    ...
    window.dispatchEvent(new CustomEvent("vendorCreated", { detail: newVendor }));
    
    // Call the callback with the new vendor object
    if (onVendorCreated) {
