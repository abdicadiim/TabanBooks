...
import { billsAPI, purchaseOrdersAPI, vendorsAPI } from "../../../services/api";
...
const REQUIRED_FIELDS = ["Bill#", "Vendor Name", "Date", "Amount"];

const FIELD_ALIASES: Record<string, string[]> = {
  "Bill#": ["Bill Number", "BillNo", "Bill Number"],
  Date: ["Bill Date"],
  "Due Date": ["DueDate"],
  "Vendor Name": ["Vendor", "VendorName"],
  "Reference#": ["Reference", "Reference Number"],
  "Order#": ["Order Number", "PO Number", "Purchase Order#"],
  "Payment Terms": ["Terms", "PaymentTerms"],
  Amount: ["Total", "Bill Amount", "Balance"],
...
const normalizeStatus = (value: any): string => {
  ...
  if (["draft", "open", "paid", "partially paid", "overdue", "void", "cancelled"].includes(normalized)) return normalized;
    ...
    try {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        try {
          ...
          const vendorName = getRowValue(row, "Vendor Name");
          const amountRaw = getRowValue(row, "Amount");
          const billDate = formatDateForApi(getRowValue(row, "Date"));

          if (!billNumber || !vendorName || !amountRaw || !billDate) {
            skippedCount += 1;
            continue;
          }
          ...
          const orderNumber = getRowValue(row, "Order#");
          const matchedPO = purchaseOrders.find((po: any) =>
            orderNumber && String(po?.purchaseOrderNumber || "").trim().toLowerCase() === orderNumber.trim().toLowerCase()
          );
          ...
          const payload: any = {
            billNumber,
            date: billDate,
            dueDate: formatDateForApi(getRowValue(row, "Due Date") || billDate),
            vendor: vendor?._id || vendor?.id,
            vendorName: vendor?.displayName || vendor?.name || vendorName,
            referenceNumber: getRowValue(row, "Reference#"),
            orderNumber: orderNumber || undefined,
            paymentTerms: getRowValue(row, "Payment Terms") || "Due on Receipt",
            ...
            fromPurchaseOrder: Boolean(matchedPO),
            purchaseOrderId: matchedPO?._id || matchedPO?.id || undefined,
            items: [
              {
                description: getRowValue(row, "Notes") || "Imported bill",
                quantity: 1,
                unitPrice: amount,
                total: amount,
              },
    ...
    <div className="flex flex-col h-screen bg-white">
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50">
        <div className="flex items-center gap-4">
          <h1 className="text-lg font-semibold text-gray-900 m-0">{currentStep === 1 ? "Bills - Select File" : currentStep === 2 ? "Map Fields" : "Preview"}</h1>
        ...
        <button onClick={() => navigate("/purchases/bills")} className="p-2 bg-transparent border-none cursor-pointer"><X size={20} className="text-red-600" /></button>
      </div>
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">Bill Details</div>
      ...
      <div className="px-8 py-4 border-t border-gray-200 flex items-center justify-between bg-white">
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate("/purchases/bills")} className="px-4 py-2 text-sm border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-50">Cancel</button>
