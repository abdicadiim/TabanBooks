...
type UserRecord = {
  ...
  isActive?: boolean;
  avatar?: string;
  accessibleLocations?: Array<LocationRecord | EntityId>;
  defaultBusinessLocation?: LocationRecord | EntityId | null;
  defaultWarehouseLocation?: LocationRecord | EntityId | null;
  [key: string]: any;
};
...
const getEntityId = (value: { _id?: EntityId; id?: EntityId } | EntityId | null | undefined): EntityId | undefined => {
...
export default function UsersPage() {
  ...
  const [accessibleLocations, setAccessibleLocations] = useState<EntityId[]>([]);
  ...
  const [userDetails, setUserDetails] = useState<UserRecord | null>(null);
  ...
  const inviteDropdownButtonRef = useRef<HTMLButtonElement | null>(null);
        ...
        if (response && response.success) {
          ...
          if (response.data.accessibleLocations && response.data.accessibleLocations.length > 0) {
            ...
            if (locationsResponse && locationsResponse.success) {
              const allLocations: LocationRecord[] = locationsResponse.data || [];
              const userAccessibleLocations = allLocations.filter((loc) =>
                locationIds.some((id: LocationRecord | EntityId) => getEntityId(loc) === getEntityId(id))
              );
      ...
      if (response.success) {
        ...
        if (userId) {
          ...
          if (userData?.accessibleLocations && Array.isArray(userData.accessibleLocations) && userData.accessibleLocations.length > 0) {
            const locationIds = userData.accessibleLocations.map((loc: LocationRecord | EntityId) => {
          ...
          Promise.all([
            usersAPI.getById(userId).then(userResponse => {
              if (userResponse && userResponse.success) {
                ...
                setSelectedUser((prev) => prev ? ({ ...prev, ...userResponse.data }) : prev);
              }
  ...
  const handleLocationSelect = (locationId: EntityId) => {
  ...
  const handleLocationRemove = (locationId: EntityId) => {
    setAccessibleLocations(accessibleLocations.filter(id => id !== locationId));
    if (defaultBusinessLocation === locationId) {
      setDefaultBusinessLocation("");
    }
  ...
  const handleSelectAllLocations = () => {
    const allLocationIds = filteredLocations
      .map((loc) => getEntityId(loc))
      .filter((id): id is EntityId => id !== undefined);
    // Toggle: if all filtered locations are selected, deselect them; otherwise select all
    const allSelected = filteredLocations.length > 0 && filteredLocations.every((loc) => {
      const locationId = getEntityId(loc);
      return locationId !== undefined && accessibleLocations.includes(locationId);
    });
    if (allSelected) {
  ...
  const accessibleLocationObjects = locations.filter(location =>
    getEntityId(location) !== undefined && accessibleLocations.includes(getEntityId(location) as EntityId)
  );
  ...
  const handleInviteAgain = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (userResponse && userResponse.success) {
          setUserDetails(userResponse.data);
          setSelectedUser((prev) => prev ? { ...prev, ...userResponse.data, status: userResponse.data.status } : prev);
        }
      } else {
