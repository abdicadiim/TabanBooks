...
export default function CreateAccountModal({ accountType, onClose, onSave }: CreateAccountModalProps) {
  ...
  const [showTabanExpense, setShowTabanExpense] = useState(false);
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={showTabanExpense}
                    onChange={(e) => setShowTabanExpense(e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                ...
                {showTabanExpense && (
                  <label className="flex items-center gap-2 cursor-pointer mt-2 ml-6">
                    <input
                      ...
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                    <span className="text-sm text-gray-700">Show as an active account in Taban Books Expense</span>
                    <Info size={14} className="text-gray-400" />
                  </label>
                )}
              </div>
