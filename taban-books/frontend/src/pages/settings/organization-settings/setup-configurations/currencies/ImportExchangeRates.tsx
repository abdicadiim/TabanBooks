...
export default function ImportExchangeRates() {
  ...
  const downloadSampleFile = () => {
    ...
    XLSX.writeFile(workbook, "currency_import_sample.xlsx");
  };
  ...
  const handleImport = async () => {
    ...
    setIsImporting(true);
    try {
      const currenciesResponse = await fetch(`${API_BASE_URL}/settings/currencies`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const currenciesData = await currenciesResponse.json();
      if (!currenciesResponse.ok || !currenciesData.success || !Array.isArray(currenciesData.data)) {
        throw new Error(currenciesData.message || "Failed to load existing currencies.");
      }

      const existingByCode = new Map<string, any>();
      currenciesData.data.forEach((currency: any) => {
      ...
      for (const row of readyRows) {
        const existingCurrency = existingByCode.get(row.code);
        const payload: any = {
          code: row.code,
          name: row.name,
          symbol: row.symbol,
        ...
        try {
          const response = await fetch(
            existingCurrency
              ? `${API_BASE_URL}/settings/currencies/${existingCurrency._id || existingCurrency.id}`
              : `${API_BASE_URL}/settings/currencies`,
            {
              method: existingCurrency ? "PUT" : "POST",
              headers: {
          ...
          currencyId = String(data?.data?._id || data?.data?.id || existingCurrency?._id || existingCurrency?.id || "");

          if (currencyId && row.exchangeRate && row.exchangeRate > 0) {
            const rateResponse = await fetch(`${API_BASE_URL}/settings/currencies/${currencyId}/exchange-rates`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            ...
            if (!rateResponse.ok || !rateData.success) {
              throw new Error(rateData.message || "Failed to add exchange rate");
            }
          }
