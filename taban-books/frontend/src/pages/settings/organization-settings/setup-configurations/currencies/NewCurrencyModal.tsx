...
export default function NewCurrencyModal({ onClose, onSave }: NewCurrencyModalProps) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center pt-4 pb-4 overflow-y-auto z-[10000]"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[calc(100vh-2rem)] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-2 text-sm font-medium text-white bg-[#156372] rounded-lg hover:bg-[#0f4f5c] transition-colors shadow-sm"
          >
            Save
          </button>
        </div>
