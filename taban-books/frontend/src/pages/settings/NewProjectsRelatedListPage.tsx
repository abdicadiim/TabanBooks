...
export default function NewProjectsRelatedListPage() {
  const navigate = useNavigate();
  const [relatedListName, setRelatedListName] = useState("");
  const [visibility, setVisibility] = useState("only-me");
  const [script, setScript] = useState(`/* This is a sample function. It will display the project details. */
headerData = List();
headerData.add({"label":"Project Name"});
headerData.add({"label":"Customer Name"});
headerData.add({"label":"Status"});
headerData.add({"label":"Rate","align":"right"});
details = Map();
details.put("project_name",{"value":project.get("name"),"isExternal":true,"link":"https://books.tabanbooks.com/app#/projects/" + project.get("project_id")});
details.put("customer_name",{"value":project.get("customer_name"),"isExternal":true,"link":"https://books.tabanbooks.com/app#/contacts/" + project.get("customer_id")});
details.put("status",{"value":project.get("status")});
details.put("rate",{"value":project.get("rate")});
listData = List();
...
return resultMap;`);
  ...
  const delugeComponents = [
    ...
    },
    {
      ...
      items: ["webhook", "taban books integration"]
    }
  ];
  ...
  return (
    <div className="p-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">New Related List - Projects</h1>
        <button
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="space-y-6">
          ...
          {/* Deluge Script Editor Section */}
          <div>
            <div className="flex items-center gap-2 mb-3">
              <Lightbulb size={16} className="text-yellow-500" />
              <p className="text-sm text-gray-600">
                Enter a script to fetch data from third party services and view them from within Taban Books.
              </p>
            </div>

            <div className="flex gap-4">
              {/* Left Sidebar - Code Components */}
              ...
              <div className="flex-1 border border-gray-300 rounded-lg overflow-hidden">
                ...
                <div className="bg-gray-50 border-b border-gray-200 px-4 py-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-gray-600">Deluge by Taban Books Creator</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="bg-blue-50 border-b border-blue-200 px-4 py-2">
                  <code className="text-sm text-gray-900 font-mono">
                    Map My_Related_List( Map project, Map organization, Map user, Map page context ) {'{'}
                  </code>
                </div>
