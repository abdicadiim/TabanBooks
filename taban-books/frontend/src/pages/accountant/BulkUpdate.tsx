      ...
      {open && (
        ...
        >
          ...
          <div style={{ maxHeight: 240, overflowY: "auto", padding: 6 }}>
            {filtered.length === 0 ? (
              <div
                style={{ padding: "8px 10px", color: "#64748b", fontSize: 13 }}
              >
                No results
              </div>
            ) : (
                  ...
                  <React.Fragment key={o.id}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "8px 10px",
                        background:
                          hoveredId === o.id ? "#f1f5f9" : "transparent",
                        color: "#334155",
                        cursor: "pointer",
                        fontSize: 13,
                        display: "flex",
                    ...
                    >
                      <span>{o.label}</span>
                      {isActive && (
                        <span style={{ fontSize: 14, color: "#1a9b64" }}>
                          ✓
                        </span>
                      )}
                    </button>
...
export default function BulkUpdate() {
  ...
  const locationOptions = useMemo<OptionItem[]>(
    () => [{ id: "", label: "All Locations" }, ...locations],
    [locations],
  );

  const selectedAccount = selectableAccounts.find(
    (a) => a._id === filters.accountId,
  );
  const selectedNewAccount = selectableAccounts.find(
    (a) => a._id === newAccountId,
  );
  const selectedLocation = locationOptions.find(
    (l) => l.id === filters.locationId,
  );
  ...
  const loadBase = async () => {
    ...
    try {
      ...
      const [accountsRes, historyRes, vendorsRes, customersRes, locationsRes] =
        results.map((r) =>
          r.status === "fulfilled"
            ? r.value
      ...
      const uniqueContacts = Array.from(
        new Set([...vendors, ...customers].map((n) => String(n).trim())),
      )
        .filter(Boolean)
      ...
      if (failedCount > 0) {
        toast.error(
          `Some data failed to load (${failedCount} requests failed). Check console for details.`,
        );
      }
    } catch (err: any) {
  ...
  const searchTransactions = async () => {
    ...
    try {
      ...
      const res = await accountantAPI.previewBulkUpdateTransactions({
        ...
        locationName:
          selectedLocation?.label && filters.locationId
      ...
      toast.success(
        `Found ${res.data?.transactions?.length || 0} transactions.`,
      );
    } catch (error: any) {
    ...
    >
      <div
        style={{
          ...
          margin: "0 auto",
          display: "grid",
          gridTemplateColumns: hasSearched ? "2fr 1fr" : "1fr",
          gap: 14,
        }}
      >
        {!hasSearched ? (
          ...
          >
            <div style={{ maxWidth: 900, textAlign: "center" }}>
              <div style={{ fontSize: 68, lineHeight: 1, color: "#7c3aed" }}>
                [=]
              </div>
              <h2
                style={{
              ...
              <p style={{ margin: "0 0 16px", fontSize: 18, color: "#111827" }}>
                Filter transactions (Invoices, Credit Notes, Purchase Orders,
                Expenses, Bills, Vendor Credits) and bulk-update its accounts
                with a new account
              </p>
              ...
              >
                Bulk-updating accounts in transactions will cause significant
                changes to the financial data of your business. We recommend
                that you do this with the assistance of an accountant.
              </div>
                    ...
                    results.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: "8px 10px" }}>
                            {r.displayNumber || r.transactionId.slice(-6)}
                          </td>
                          <td style={{ padding: "8px 10px" }}>
                            {r.date
            ...
            >
              ...
              <button
                ...
                style={{
                  padding: "8px 14px",
                  border: "none",
                  background: selected.length ? "#156372" : "#94a3b8",
                  color: "white",
                  borderRadius: 8,
            ...
            >
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {history.length === 0 ? (
                    <tr>
                      <td colSpan={3} style={{ padding: 10, color: "#64748b" }}>
                        No bulk updates yet.
                      </td>
                    </tr>
                  ) : (
                    history.map((h) => (
                      <tr
                        key={h._id}
                        onClick={() => loadHistoryDetail(h._id)}
                        style={{
                          borderBottom: "1px solid #f1f5f9",
                          cursor: "pointer",
                          background:
                            selectedHistoryId === h._id ? "#f8fafc" : "white",
                        }}
                      >
                        <td style={{ padding: "8px 10px" }}>
            ...
            {selectedHistoryDetail && (
              ...
              >
                ...
                <div style={{ fontWeight: 600 }}>
                  {selectedHistoryDetail.description ||
                    `${selectedHistoryDetail.fromAccountName} -> ${selectedHistoryDetail.toAccountName}`}
                </div>
          ...
          >
            ...
            <div style={{ padding: 24 }}>
              <p style={{ margin: "0 0 18px", color: "#156372", fontSize: 16 }}>
                Select an account and enter your ranges to filter your
                transaction
              </p>
