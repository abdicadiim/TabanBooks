    ...
    <>
      <div className="relative z-[70] h-14 bg-white border-b border-slate-200">
        <div className="h-full px-3 md:px-5 flex items-center justify-between gap-3">
          ...
          <div className="flex-1 max-w-[700px] hidden sm:block">
            <div className="relative" ref={searchRef}>
              ...
              <button
                ...
                className="absolute left-9 top-1/2 -translate-y-1/2 flex items-center gap-1 z-10"
              >
                {searchDropdownOpen ? (
              ...
              <input
                className="w-full h-9 rounded-xl border border-slate-200 bg-slate-50 pl-[60px] pr-16 text-[13px] text-slate-800 placeholder:text-slate-400 outline-none focus:bg-white focus:border-blue-300"
                placeholder={`Search in ${searchScope} ( / )`}
              />
              <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] text-slate-400 border border-slate-200 bg-white px-2 py-0.5 rounded-md">
                /
              </span>

              {/* Search Dropdown */}
              {searchDropdownOpen && (
                <div className="absolute left-0 top-full mt-2 w-[300px] bg-white border border-slate-200 rounded-lg shadow-lg z-[100] max-h-[500px] overflow-y-auto">
                  <div className="py-2">
                    {searchOptions.map((option) => (
                      <button
                        key={option.label}
                        onClick={() => handleSearchScopeChange(option)}
                        className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between hover:bg-blue-50 ${searchScope === option.label ? "bg-blue-50" : ""
                          }`}
                      >
                        <span className={searchScope === option.label ? "text-blue-600 font-medium" : "text-slate-700"}>
                          {option.label}
                        </span>
                        {searchScope === option.label && (
                    ...
                    <button
                      onClick={() => {
                        setSearchDropdownOpen(false);
                        // Handle Search across Taban Books
                      }}
                      className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center justify-between"
                    >
                      <div className="flex items-center gap-2">
                        <Search size={14} className="text-slate-400" />
                        <span>Search across Taban Books</span>
                      </div>
