...
import { X, ChevronDown, ChevronLeft, ChevronRight, Edit, Info } from "lucide-react";
import toast from "react-hot-toast";

export default function ImportQuantityMapFields() {
  ...
  const [isDateFieldDropdownOpen, setIsDateFieldDropdownOpen] = useState({});
  ...
  const handleNext = () => {
    ...
    if (missingFields.length > 0) {
      // Just warn but allow proceeding, or silence completely. 
      // User asked to "make it go", so we proceed.
      console.warn(`Missing required fields: ${missingFields.map(f => f.label).join(", ")}`);
    }

    navigate("/inventory/import/quantity/preview", {
      state: {
        file,
        characterEncoding,
        type,
