// src/features/inventory/NewCustomView.jsx
import React, { useState, useMemo, useRef, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { X, Plus, Star, Search, Check, GripVertical, ChevronDown, ChevronUp, Lock, Users, Globe, Edit2 } from "lucide-react";
...
export default function NewCustomView() {
  ...
  const [formData, setFormData] = useState({
    ...
    selectedColumns: ["Date", "Type"],
    visibility: "onlyMe", // "onlyMe", "selectedUsers", "everyone"
    selectedType: "Users", // "Users" or "Roles"
    selectedUsers: [], // Array of user objects: { name, email, initial }
    selectedRoles: [],
    criteriaPattern: "", // Pattern like "(1 AND 2)"
  });
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Handle different response formats: array directly or wrapped in data property
        const users = Array.isArray(response) ? response : (response?.data || []);
        console.log("Parsed users:", users); // Debug log
        // Transform users to match expected format: { id, name, email, initial }
        const transformedUsers = users.map((user) => ({
          id: user._id || user.id,
          name: user.name || user.displayName || user.email || "",
          email: user.email || "",
          initial: (user.name || user.displayName || user.email || "U").charAt(0).toUpperCase(),
        }));
        console.log("Transformed users:", transformedUsers); // Debug log
        setFetchedUsers(transformedUsers);
      } catch (error) {
        // Only log to console, don't show toast for 404 (endpoint not implemented yet)
        if (error.status === 404 || error.message?.includes("404")) {
          console.log("Users API endpoint not available yet. Users will be empty until backend implements /api/users");
        } else {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      ...
      try {
        ...
        // Keep roles as objects so we can display name and use roleName/id for comparison
        // Don't transform to strings - keep the full objects
        setFetchedRoles(roles);
        console.log("Set fetchedRoles:", roles); // Debug log
      } catch (error) {
        // Only log to console, don't show toast for 404 (endpoint not implemented yet)
        if (error.status === 404 || error.message?.includes("404")) {
          console.log("Roles API endpoint not available yet. Roles will be empty until backend implements /api/roles");
        } else {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (userRoleDropdownRef.current && !userRoleDropdownRef.current.contains(event.target)) {
  ...
  // Available columns for inventory adjustments
  ...
  const [comparatorDropdownOpen, setComparatorDropdownOpen] = useState({});
  ...
  const [logicalOperators, setLogicalOperators] = useState({}); // Store AND/OR between criteria

  // Generate criteria pattern
  useEffect(() => {
    if (formData.criteria.length > 1) {
      const pattern = formData.criteria
        .map((_, index) => {
          const operator = logicalOperators[index] || "AND";
          return index === 0 ? `(${index + 1}` : ` ${operator} ${index + 1}`;
        })
        .join("") + ")";
      setFormData((prev) => ({ ...prev, criteriaPattern: pattern }));
    } else {
      setFormData((prev) => ({ ...prev, criteriaPattern: "" }));
    }
  }, [formData.criteria.length, logicalOperators]);

  const getFilteredComparators = (criterionId) => {
    const search = comparatorSearch[criterionId] || "";
    return comparatorOptions.filter((comp) =>
      comp.toLowerCase().includes(search.toLowerCase())
    );
  };

  const getFilteredFields = (criterionId) => {
    const search = fieldSearch[criterionId] || "";
  ...
  const removeCriterion = (id) => {
    ...
    // Remove logical operator for this index
    const index = formData.criteria.findIndex((c) => c.id === id);
    if (index > 0) {
      const newOperators = { ...logicalOperators };
      delete newOperators[index - 1];
      // Shift operators
      Object.keys(newOperators).forEach((key) => {
        if (parseInt(key) >= index) {
          newOperators[parseInt(key) - 1] = newOperators[key];
  ...
  const toggleLogicalOperator = (index) => {
  ...
  const moveColumnToAvailable = (column) => {
    // Don't allow removing "Date" and "Type" columns
    if (column === "Date" || column === "Type") return;
    setFormData((prev) => ({
      ...prev,
  ...
  const handleAddUser = (user) => {
    if (!formData.selectedUsers.find((u) => u.id === user.id || u.name === user.name)) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleAddRole = (role) => {
    ...
    const roleName = typeof role === 'string' ? role : (role.name || role.roleName || role.id);
    if (!formData.selectedRoles.includes(roleName)) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleRemoveUser = (userIdOrName) => {
    setFormData((prev) => ({
      ...prev,
      selectedUsers: prev.selectedUsers.filter((u) => u.id !== userIdOrName && u.name !== userIdOrName),
    }));
  };
  ...
  const filteredUsers = useMemo(() => {
    ...
    return fetchedUsers;
  }, [fetchedUsers, accessDetailSearch]);

  const handleSubmit = (e) => {
    e.preventDefault();
    // Handle save logic here - save to backend
    console.log("Saving custom view:", formData);
    // TODO: Call API to save custom view
    navigate("/inventory");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ paddingTop: "56px" }}>
        ...
        >
            ...
            <div className="flex gap-4 items-end">
              ...
              <div className="flex items-center gap-2 pb-2">
                ...
                >
                  <Star
                    size={16}
                    className={formData.markAsFavorite ? "text-amber-400 fill-amber-400" : "text-gray-400"}
                  />
                  Mark as Favorite
                </label>
              </div>
              ...
              {formData.criteria.map((criterion, index) => (
                <div key={criterion.id}>
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="relative flex-1" ref={(el) => (fieldDropdownRefs.current[criterion.id] = el)}>
                      <input
                        type="text"
                    ...
                    <div className="relative flex-1" ref={(el) => (comparatorDropdownRefs.current[criterion.id] = el)}>
                      <input
                        type="text"
                        ...
                        className={`px-2 py-1.5 text-sm border border-gray-200 rounded w-full cursor-pointer ${
                          !criterion.field ? "bg-gray-100 cursor-not-allowed" : "bg-white"
                        }`}
                    ...
                    <div className="flex items-center gap-2 mt-2 ml-7">
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-medium rounded ${
                          logicalOperators[index] === "OR"
                            ? "bg-blue-100 text-blue-700 border border-blue-300"
                            : "bg-gray-100 text-gray-700 border border-gray-300"
                        }`}
                      >
                        {logicalOperators[index] === "OR" ? "OR" : "AND"}
                      </button>
                    </div>
                  )}
                </div>
            ...
            {formData.criteria.length > 1 && (
              <div className="mt-4 flex items-center gap-2">
                ...
                {editingPattern ? (
                  <input
                    type="text"
                    value={formData.criteriaPattern}
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, criteriaPattern: e.target.value }))
                    }
                    onBlur={() => setEditingPattern(false)}
                    className="px-2 py-1 text-sm border border-gray-200 rounded"
                    autoFocus
                  />
                ) : (
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-700">{formData.criteriaPattern || "(1 AND 2)"}</span>
                    <button
                      type="button"
                ...
                <div className="flex flex-col gap-1 max-h-[300px] overflow-y-auto">
                  {formData.selectedColumns.map((column) => (
                    <div
                      ...
                      className={`p-2 text-sm text-gray-900 flex items-center gap-2 ${
                        column === "Date" || column === "Type" ? "cursor-default" : "cursor-pointer hover:bg-gray-50"
                      }`}
                    >
                      <GripVertical size={16} className="text-gray-500" />
                      {column}
                      {(column === "Date" || column === "Type") && (
            ...
            <div className="space-y-4">
              ...
              {formData.visibility === "selectedUsers" && (
                <div className="mt-4 space-y-4">
                  {/* Combined Users/Roles Selection */}
                  <div>
                    <div className="flex gap-2 items-center">
                          ...
                          <div className="absolute z-50 left-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg min-w-[120px]">
                            <div
                              className={`px-3 py-2 text-sm cursor-pointer ${
                                (!formData.selectedType || formData.selectedType === "Users") ? "bg-blue-50 text-blue-600" : "hover:bg-gray-50 text-gray-700"
                              }`}
                              onClick={() => {
                            ...
                            <div
                              className={`px-3 py-2 text-sm cursor-pointer ${
                                formData.selectedType === "Roles" ? "bg-blue-50 text-blue-600" : "hover:bg-gray-50 text-gray-700"
                              }`}
                              onClick={() => {
                        ...
                        {usersDropdownOpen && formData.selectedType !== "Roles" && (
                          <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                            {loadingUsers ? (
                              ...
                              const isSelected = formData.selectedUsers.find((u) => u.id === user.id || u.name === user.name);
                              const isHighlighted = selectedUserToAdd?.id === user.id || selectedUserToAdd?.name === user.name;
                              return (
                                <div
                                  key={user.id || user.name}
                                  className={`px-3 py-2 text-sm cursor-pointer flex items-center justify-between ${
                                    isSelected ? "bg-blue-50" : isHighlighted ? "bg-teal-50" : "hover:bg-gray-50"
                                  }`}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (!isSelected) {
                        ...
                        {rolesDropdownOpen && formData.selectedType === "Roles" && (
                          <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                            {loadingRoles ? (
                              ...
                              // Handle both string and object formats
                              const roleName = typeof role === 'string' ? role : (role.name || role.roleName || role.id);
                              const roleDisplayName = typeof role === 'string' ? role : (role.name || role.roleName || role.id);
                              const isSelected = formData.selectedRoles.includes(roleName);
                              const isHighlighted = selectedRoleToAdd && (
                                (typeof selectedRoleToAdd === 'string' && selectedRoleToAdd === roleName) ||
                                (typeof selectedRoleToAdd === 'object' && (selectedRoleToAdd.name === roleName || selectedRoleToAdd.roleName === roleName || selectedRoleToAdd.id === roleName))
                              );
                              return (
                                <div
                                  key={roleName}
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (formData.selectedType === "Roles") {
                            if (selectedRoleToAdd) {
                              handleAddRole(selectedRoleToAdd);
                              setSelectedRoleToAdd(null);
                              setRolesDropdownOpen(false);
                            } else {
                              setRolesDropdownOpen(!rolesDropdownOpen);
                            ...
                            if (selectedUserToAdd) {
                              handleAddUser(selectedUserToAdd);
                              setSelectedUserToAdd(null);
                              setUsersDropdownOpen(false);
                            } else {
                              setUsersDropdownOpen(!usersDropdownOpen);
                        ...
                        className={`px-3 py-2 text-sm text-white border rounded-r-md flex items-center gap-1.5 ${
                          (formData.selectedType === "Roles" && selectedRoleToAdd) || 
                          (formData.selectedType === "Users" && selectedUserToAdd) 
                            ? "opacity-100" : "opacity-90"
                        }`}
                        style={{ 
                          background: (formData.selectedType === "Roles" && selectedRoleToAdd) || 
                                     (formData.selectedType === "Users" && selectedUserToAdd)
                        ...
                        onMouseLeave={(e) => {
                          if ((formData.selectedType === "Roles" && selectedRoleToAdd) || 
                              (formData.selectedType === "Users" && selectedUserToAdd)) {
          ...
          <div className="pt-5 border-t border-gray-200 flex justify-end gap-3">
            <button
              type="button"
              onClick={handleClose}
              className="px-4 py-2 text-sm font-medium rounded-md border border-gray-200 cursor-pointer bg-white text-gray-900 hover:bg-gray-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-2 text-sm font-medium rounded-md border-none cursor-pointer text-white transition-all"
