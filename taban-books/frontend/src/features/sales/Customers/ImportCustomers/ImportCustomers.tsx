    ...
    if (file) {
      ...
      const uploadFile = async () => {
        try {
          ...
          console.log("File auto-uploaded to documents");
        } catch (err) {
          console.warn("Auto-upload failed (will try again during import):", err);
        }
      };
      uploadFile();
    }
    ...
    if (currentStep === "configure") {
      try {
        ...
        if (headers && headers.length > 0) {
          ...
          Object.keys(variations).forEach(tabanField => {
            if (!newMappings[tabanField]) {
              const matches = variations[tabanField];
              const foundHeader = headers.find(h =>
                matches.some(m => h.toLowerCase() === m.toLowerCase())
              );
              if (foundHeader) {
                newMappings[tabanField] = foundHeader;
              }
            }
          });
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log("Import file stored in database successfully");
      } catch (docError) {
        console.warn("Failed to store import file in documents, but proceeding with data import:", docError);
      }
      ...
      console.log(`Starting import of ${rows.length} customers...`);
      ...
      if (importedCount > 0) {
        alert(`Successfully imported ${importedCount} customer(s).${skippedCount > 0 ? `\n${skippedCount} record(s) failed or were skipped.` : ''}`);
        navigate("/sales/customers");
      } else {
        alert(`Failed to import any customers. Please check the file format.\nErrors: ${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '\n...' : ''}`);
      }
    } catch (error) {
      console.error("Error in handleImport:", error);
      setIsLoading(false);
      alert("Failed to process the requested import. Please try again.");
    }
  };
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-600 mt-1">
                      Retains the customers in Taban Books and does not import the duplicates in the import file.
                    </div>
                  </div>
                </label>
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-600 mt-1">
                      Imports the duplicates in the import file and overwrites the existing customers in Taban Books.
                    </div>
                  </div>
                </label>
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-600 mt-1">
                      Imports the duplicates in the import file and adds them as new customers in Taban Books.
                    </div>
                  </div>
                </label>
              </div>
