    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 pt-16">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
        ...
        <div className="flex items-center justify-start gap-3 p-6 border-t border-gray-200">
          <button
            type="button"
            className="px-6 py-2 bg-[#0f766e] text-white rounded-md text-sm font-medium cursor-pointer hover:bg-[#115e59] transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed disabled:cursor-not-allowed inline-flex items-center gap-2"
            onClick={onConfirm}
            disabled={!selectedVendor || isConfirming}
          >
            {isConfirming ? "Linking..." : "Link"}
          </button>
          <button
            type="button"
            className="px-6 py-2 bg-white border border-gray-300 text-gray-700 rounded-md text-sm font-medium cursor-pointer hover:bg-gray-50 transition-colors disabled:cursor-not-allowed disabled:opacity-60"
            onClick={onClose}
