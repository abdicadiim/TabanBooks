...
export default function ImportPayments() {
  ...
  const headerAliases = {
    ...
    "Date": ["payment date", "received date", "date received", "transaction date"],
    "Customer Name": ["customer", "customer id", "client", "client name", "company", "display name", "name"],
    ...
    "Amount": ["amount received", "received amount", "payment amount", "total", "value"],
    "Deposit To": ["deposit account", "deposit to account", "bank account", "account", "paid to"],
    "Description": ["notes", "memo", "remark", "details"],
    "Invoice Number": ["invoice", "invoice no", "invoice #", "invoice id", "sales invoice"],
      ...
      if (selectedFile) {
        try {
          ...
          const requiredFields = autoGenerateNumbers
            ...
            : ["Payment #", "Customer Name", "Date", "Amount"];
          let unmappedCount = 0;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const normalizePaymentMode = (rawMode) => {
        return getPaymentModeLabel(rawMode, "Cash");
      };
      ...
      for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
        ...
        try {
          ...
          const paymentDate = getValue("Date");
          ...
          const invoiceNumberValue = getValue("Invoice Number");
          const resolvedInvoices = resolveInvoicesForRow(invoiceNumberValue, customerId);
          ...
          const paymentData = {
            ...
            paymentMode: normalizePaymentMode(getValue("Payment Mode")),
            amount: parsedAmount,
      ...
      if (importedCount > 0) {
        alert(`Successfully imported ${importedCount} payment(s).${skippedCount > 0 ? ` ${skippedCount} record(s) skipped.` : ''}`);
        navigate("/sales/payments-received");
      } else {
        alert(`No payments were imported.${errors.length > 0 ? ` ${errors.slice(0, 3).join(" ")}` : ""}`);
      }
    } catch (error) {
      console.error("Error importing payments:", error);
      alert("Error importing payments. Please check the file format and try again.");
    }
  };
        ...
        {currentStep === "mapFields" && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="space-y-4">
              ...
              {["Payment #", "Date", "Customer Name", "Reference Number", "Payment Mode", "Amount", "Deposit To", "Description", "Invoice Number", "Currency"].map((field) => (
                <div key={field} className="grid grid-cols-12 gap-4 items-center">
                  <div className="col-span-4 text-sm font-medium text-gray-700">
                    {field} <span className="text-red-500">{(["Customer Name", "Date", "Amount"].includes(field) || (!autoGenerateNumbers && field === "Payment #")) && "*"}</span>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="grid grid-cols-3 gap-6 mb-8">
              <div className="border border-green-200 bg-green-50 rounded-lg p-4 text-center">
                ...
                <div className="text-sm font-medium text-green-800">Payments Ready</div>
              </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <p>Click the "Import" button below to verify and import your payments data. Make sure all required fields are mapped correctly.</p>
              </div>
            </div>
          </div>
        )}
