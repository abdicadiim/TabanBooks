...
export default function SalespersonDropdown({
  ...
  return (
    <div className="relative w-[280px]" ref={dropdownRef}>
      <div
        className="w-full h-9 px-3 border border-gray-300 rounded text-sm text-gray-700 flex justify-between items-center bg-white cursor-pointer"
        onClick={onToggle}
      >
          ...
          <div className="max-h-60 overflow-y-auto">
            {salespersons.length > 0 ? (
              salespersons.map((salesperson, idx) => (
                <div
                  key={salesperson.id || salesperson._id || `sp-${idx}`}
