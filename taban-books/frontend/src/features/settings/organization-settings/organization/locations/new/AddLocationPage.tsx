...
export default function AddLocationPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && Array.isArray(data.data) && data.data.length > 0) {
            ...
            const preferredUser =
              mappedUsers.find((u: any) => String(u.id) === String(currentUser?.id)) || mappedUsers[0];

            setFormData(prev => ({
              ...prev,
              primaryContact: preferredUser?.id || "",
              locationAccess: prev.locationAccess.length > 0 ? prev.locationAccess : [{
  ...
  // Load all users for Location Access dropdown
  useEffect(() => {
  ...
  useEffect(() => {
    const loadParentLocations = async () => {
      // Load parent locations for Warehouse type or when child location checkbox is checked
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          // Response is not JSON, but it's OK status
          navigate('/settings/locations');
        }
      } else {
