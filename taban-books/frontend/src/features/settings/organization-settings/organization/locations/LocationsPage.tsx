...
export default function LocationsPage() {
  ...
  useEffect(() => {
    const checkEnabled = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/settings/organization/locations/enabled`, {
          headers: {
  ...
  const handleEnableLocations = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/settings/organization/locations/enable`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
