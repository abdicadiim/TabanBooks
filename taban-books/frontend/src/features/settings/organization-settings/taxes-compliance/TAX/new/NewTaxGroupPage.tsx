...
export default function NewTaxGroupPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const taxesResponse = await fetch(`${API_BASE_URL}/settings/taxes`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        const taxesData = await taxesResponse.json();
        if (!taxesResponse.ok || !taxesData.success) {
          throw new Error(taxesData.message || "Failed to load taxes");
        }

        const normalTaxes = (taxesData.data || [])
          .filter(
            (tax: any) =>
        ...
        if (isEditMode && id) {
          const groupResponse = await fetch(`${API_BASE_URL}/settings/taxes/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          const groupData = await groupResponse.json();
          if (!groupResponse.ok || !groupData.success || !groupData.data) {
            throw new Error(groupData.message || "Failed to load tax group");
          }

          const group = groupData.data;
          const isGroup =
            group.description === TAX_GROUP_MARKER ||
            (Array.isArray(group.groupTaxes) && group.groupTaxes.length > 0);
